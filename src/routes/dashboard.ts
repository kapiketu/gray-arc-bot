// ─────────────────────────────────────────────────────────────────────────────
// FOUNDER DASHBOARD ROUTE (Input Channel 2)
// REST API endpoints for the Founder Dashboard (Website / SaaS / Mobile App).
// POST /v3/dashboard/projects              — Create a new project from Dashboard form
// GET  /v3/dashboard/projects              — List all projects
// GET  /v3/dashboard/projects/:id          — Get a single project with its state
// PUT  /v3/dashboard/projects/:id/status   — Founder updates project status
// POST /v3/dashboard/projects/:id/approve  — Founder approves → triggers Phase 5
// GET  /v3/dashboard/projects/:id/qa       — QA report
// GET  /v3/dashboard/projects/:id/logs     — Get AI logs for a project
// GET  /v3/dashboard/projects/:id/deliverables — Full deliverables package
// GET  /v3/dashboard/queue                 — Get Orchestrator queue status
// GET  /v3/dashboard/cost/:id              — Get cost report for a project
// ─────────────────────────────────────────────────────────────────────────────
import { FastifyInstance, FastifyRequest, FastifyReply } from 'fastify';
import { normalizeFromDashboard, persistNormalizedRequest } from '../services/inputNormalizer';
import { runTask, getQueueStatus, getCostReport } from '../services/orchestrator';
import { supabase } from '../db/supabaseClient';
import { ProjectType, ProjectStatus } from '../types/agent.types';
import { runPmAgent } from '../agents/pmAgent';
import {
  notifyChangesRequested,
  notifyProjectApproved,
} from '../services/notificationEngine';
import { runDeliveryPipeline } from '../services/deliveryPipeline';
...
export default async function dashboardRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/v3/dashboard/projects', async (
    ...
    try {
      ...
      const normalized = normalizeFromDashboard(body);
      ...
      await runTask(
        'pre-init',
        'InputNormalizer',
        'normalize_dashboard_input',
        body as unknown as Record<string, unknown>,
        async () => {
          projectId = await persistNormalizedRequest(normalized);
  ...
  fastify.get('/v3/dashboard/projects', async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.get('/v3/dashboard/projects/:id', async (
  ...
  fastify.put('/v3/dashboard/projects/:id/status', async (
    ...
    try {
      ...
      const validStatuses: ProjectStatus[] = ['Draft', 'Requirements', 'Planning', 'Execution', 'QA', 'Review', 'Completed', 'Cancelled'];
      ...
      if (status === 'Completed') {
        await notifyProjectApproved(id, project.name);
      ...
      if (status === 'Execution' && founderNote) {
        await notifyChangesRequested(id, project.name, founderNote);
        // Re-trigger PM Agent to create new tasks based on Founder's note
        setImmediate(async () => {
          await runTask(id, 'PM', 'handle_change_request', { founderNote }, async () => {
            ...
            const { data: stateRow } = await supabase
              ...
              .eq('project_id', id)
              .single();
            const currentState = (stateRow?.state_data as Record<string, unknown>) || {};
            await supabase.from('project_state').update({
              state_data: {
                ...currentState,
                changeRequests: [
                  ...((currentState.changeRequests as any[]) || []),
  ...
  fastify.get('/v3/dashboard/projects/:id/qa', async (
    request: FastifyRequest<{ Params: { id: string } }>,
    reply: FastifyReply
  ) => {
    try {
  ...
  fastify.get('/v3/dashboard/projects/:id/logs', async (
    request: FastifyRequest<{ Params: { id: string } }>,
    reply: FastifyReply
  ) => {
    try {
  ...
  // ─── GET: Queue Status ───────────────────────────────────────────────────
  fastify.get('/v3/dashboard/queue', async (_request: FastifyRequest, reply: FastifyReply) => {
    return reply.status(200).send({ success: true, queue: getQueueStatus() });
  });

  // ─── GET: Cost Report ───────────────────────────────────────────────────
  fastify.get('/v3/dashboard/cost/:id', async (
    request: FastifyRequest<{ Params: { id: string } }>,
    reply: FastifyReply
  ) => {
    const { id } = request.params;
    return reply.status(200).send({ success: true, cost: getCostReport(id) });
  });

  // ─── POST: Founder Approval → Triggers Phase 5 Delivery Pipeline ─────────
  fastify.post('/v3/dashboard/projects/:id/approve', async (
    request: FastifyRequest<{ Params: { id: string } }>,
    reply: FastifyReply
  ) => {
    try {
  ...
  fastify.get('/v3/dashboard/projects/:id/deliverables', async (
    request: FastifyRequest<{ Params: { id: string } }>,
    reply: FastifyReply
  ) => {
    try {
      ...
      return reply.status(200).send({
        success: true,
        deliverables: {
          generatedFiles: (state?.developerOutput as any)?.generatedFiles || [],
          deploymentFiles: (state?.deployment as any)?.deploymentFiles || [],
          deploymentInstructions: (state?.deployment as any)?.stepByStepInstructions || [],
          documentation: (state?.documentation as any)?.userManual || null,
          technicalDocs: (state?.documentation as any)?.technicalDocumentation || null,
          sops: (state?.documentation as any)?.sops || [],
          handoverPackage: (state?.handover as any)?.projectClosureReport || null,
          clientHandoverMessage: (state?.handover as any)?.clientHandoverMessage || null,
  ...
  const previewHandler = async (
    ...
    try {
      const { id } = request.params;
      const requestedPath = (request.params as any)['*'] || 'index.html';
      ...
      const state = stateRow.state_data as Record<string, unknown>;
      const generatedFiles = (state?.developerOutput as any)?.generatedFiles as Array<{ filePath: string; content: string; fileType: string }> | undefined;
      ...
      if (file.filePath.endsWith('.html')) contentType = 'text/html';
      else if (file.filePath.endsWith('.css')) contentType = 'text/css';
      else if (file.filePath.endsWith('.js')) contentType = 'application/javascript';
      else if (file.filePath.endsWith('.json')) contentType = 'application/json';
