...
async function handleChatFlow(from: string, text: string) {
  ...
  switch (session.step) {
    ...
    case 'AWAITING_SERVICES':
      ...
      await sendTextMessage(
        from,
        `Understood. Finally, please provide your **Contact Details** (e.g., Address, Phone Number, and Operating Hours). Write them down in one message.`
      );
      break;
    ...
    case 'AWAITING_DOMAIN_CHOICE':
      if (text === '1') {
        ...
        await sendTextMessage(from, `Invalid choice. Please reply with *1* for Custom Domain or *2* for Free Subdomain.`);
      }
      break;
    ...
    default:
      await db.deleteSession(from);
      await sendTextMessage(from, `Oops, something went wrong. Let's start fresh. Type anything to begin.`);
      break;
  }
}
