...
export default async function viewerRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/site/:siteId', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const previewTheme = (request.query as any).theme as string | undefined;
...
function renderPremiumWebsite(site: SiteConfig, previewTheme?: string): string {
 ...
 // --- 5 PREMIUM THEMES DICTIONARY ---
  ...
  let activeThemeKey = previewTheme || (site.theme as any)?.themeName || 'midnight_neon';
  if (!themes[activeThemeKey]) activeThemeKey = 'midnight_neon';
