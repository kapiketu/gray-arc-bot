...
import { ThemeConfig } from './stylingEngine';
 ...
 */
function processLoops(template: string, content: Record<string, any>): string {
...
export async function assembleBlocks(schema: SiteSchema, theme: ThemeConfig): Promise<string> {
  let finalBody = '';
  const blocksDir = path.join(__dirname, '../blocks');

  for (const blockId of schema.layout) {
    try {
      const blockPath = path.join(blocksDir, `${blockId}.html`);
      if (fs.existsSync(blockPath)) {
        let blockHtml = fs.readFileSync(blockPath, 'utf8');
